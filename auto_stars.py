...
# Активация всегда успешна
IS_ACTIVATED = True

def check_activation() -> bool:
    global IS_ACTIVATED
    # Всегда возвращаем True, отключаем проверку активации
    IS_ACTIVATED = True
    return True
...
class PaymentProcessor:
    ...
    async def process_payment(self, c: Cardinal, buyer_chat_id: int, username: str, stars_quantity: int, orderID: str):
        ...
        if not IS_ACTIVATED:
            c.send_message(buyer_chat_id, "❌ Бот не активирован. Транзакция не может быть выполнена. by @exfador")
            return
                ...
                if error:
                    if '406' in error and 'External message was not accepted' in error:
                        ...
                        if retry_count < max_retries:
                            logger.warning(f"Попытка {retry_count}: Ошибка 406 'External message was not accepted', повторная попытка через 5 секунд.")
                            await asyncio.sleep(5)
                            continue
                        else:
                            logger.error(f"Превышено количество попыток ({max_retries}) для заказа {orderID} из-за ошибки 406.")
                            update_stats(False, stars_quantity)
                            c.send_message(buyer_chat_id, sanitize_telegram_text("❌ Не удалось выполнить транзакцию после нескольких попыток. Пожалуйста, свяжитесь с поддержкой."))
                            if config["AUTO_REFUND"]:
                                c.account.refund(orderID)
                                c.telegram.bot.send_message(USER_ID, sanitize_telegram_text(f'Вернул пользователю: {username} деньги по причине: {error}'))
                            else:
                        ...
                        for o in user_orders:
                            if o['orderID'] == orderID and not o.get('completed', False):
                                ...
                                o['confirmed'] = False
                                break
                        c.send_message(buyer_chat_id, sanitize_telegram_text("❌ Указанный вами username не найден в Telegram. Пожалуйста, введите корректный @username для получения Stars."))
                        return
                    elif 'Не удалось декодировать JSON' in error:
                        logger.error(f"Платёж не удался для {username}: {error}")
                        user_orders = orders_info.get(buyer_chat_id, [])
                        current_order = next((o for o in user_orders if o['orderID'] == orderID and not o.get('completed', False)), None)
                        if current_order:
                        ...
                        if config["AUTO_REFUND"]:
                            c.send_message(buyer_chat_id, sanitize_telegram_text("❌ На кошельке владельца недостаточно средств для выполнения транзакции, возвращаю Вам деньги и приношу извинения."))
                            c.account.refund(orderID)
                            c.telegram.bot.send_message(USER_ID, sanitize_telegram_text(f'Вернул пользователю: {username} деньги по причине: {error}'))
                            ...
                            c.send_message(buyer_chat_id, sanitize_telegram_text("❌ На кошельке владельца недостаточно средств для выполнения транзакции. Свяжитесь с продавцом для возврата средств."))
                            send_error_with_inline_url(c, USER_ID, orderID, error)
                            c.update_lots_and_categories()
                            subcategory = c.account.get_subcategory(FunPayAPI.types.SubCategoryTypes.COMMON, SUBCATEGORY_ID)
                            my_lots = c.tg_profile.get_sorted_lots(2).get(subcategory, {})
                        ...
                        update_stats(False, stars_quantity)
                        c.send_message(buyer_chat_id, sanitize_telegram_text("❌ Ваш заказ не выполнен. Попробуйте ещё раз"))
                        if config["AUTO_REFUND"]:
                            c.account.refund(orderID)
                            c.telegram.bot.send_message(USER_ID, sanitize_telegram_text(f'Вернул пользователю: {username} деньги по причине: {error}'))
                        else:
                ...
                for attempt in range(25):
                    try:
                        with httpx.Client() as client:
                            rq = client.get(f'https://preview.toncenter.com/api/v3/traces?msg_hash={tx_hash}&include_actions=true')
                ...
                if not found_success:
                    ...
                    if config["AUTO_REFUND"]:
                        if 'Транзакция не найдена на TonViewer (404)' in check_error or 'транзакция не найдена' in check_error.lower():
                            c.send_message(buyer_chat_id, sanitize_telegram_text("❌ Не удалось подтвердить статус транзакции (404). Возвращаю вам деньги. Извините за неудобства!"))
                        else:
                            c.send_message(buyer_chat_id, sanitize_telegram_text("❌ Не удалось подтвердить статус транзакции. Возвращаю вам деньги. Извините за неудобства!"))
                        c.account.refund(orderID)
                        if 'Недостаточно средств на кошельке' in check_error:
                            ...
                            c.telegram.bot.send_message(USER_ID, sanitize_telegram_text(f'Вернул пользователю: {username} деньги по причине: {check_error}'))
                        else:
                            c.telegram.bot.send_message(USER_ID, sanitize_telegram_text(f'Вернул пользователю: {username} деньги по причине: {check_error}'))
                    else:
                        c.send_message(buyer_chat_id, sanitize_telegram_text("❌ Не удалось подтвердить статус транзакции. Свяжитесь с продавцом для возврата средств."))
                        c.telegram.bot.send_message(USER_ID, sanitize_telegram_text(f"У вас произошла ошибка с пользователем: https://funpay.com/orders/{orderID}/\nОшибка: {check_error}\nПросьба вернуть средства"))
                    return
...
📝 Поделитесь впечатлениями, буду рад! 😇
                ...
                try:
                    c.send_message(buyer_chat_id, sanitize_telegram_text(f"❌ Ваш заказ не выполнен. Причина: {str(e)}."))
        ...
        if config["AUTO_REFUND"]:
            c.account.refund(orderID)
            c.telegram.bot.send_message(USER_ID, sanitize_telegram_text(f'Вернул пользователю: {username} деньги по причине: Превышено количество попыток'))
        else:
...
def handle_new_message_text(c: Cardinal, e: NewMessageEvent, *args):
    ...
    if e.message.text.strip().lower().startswith("!бэк"):
        if current_order.get('completed', False) or current_order.get('is_canceled', False) or current_order.get('answered', False):
            return
        c.account.refund(current_order['orderID'])
        current_order['is_canceled'] = True
        c.send_message(
            buyer_chat_id,
            sanitize_telegram_text("❌ Заказ отменен, средства возвращены. Вы не можете возобновить этот заказ. Для нового заказа оформите оплату заново.")
        )
        return
    ...
    if current_order.get('username') is not None and current_order.get('auto_username', False) and not current_order.get('confirmed', False):
...
def stars_config(c: Cardinal, m: types.Message):
    ...
    try:
        ...
        activation_status = "🟢 Активирован" if IS_ACTIVATED else f"🔴 Не активирован: {HASH_MAC}"
...
def get_daily_stats():
    ...
    quantities_text = "\n".join([f"  - {q} Stars: {count} шт." for q, count in quantities.items()]) if quantities else "  - Нет данных"
        ...
        try:
            if data == "toggle_autosale":
                if RUNNING:
                    ...
                    c.telegram.bot.send_message(
                        ...
                        if not IS_ACTIVATED:
                            c.telegram.bot.send_message(
                                chat_id,
                                sanitize_telegram_text(f"❌ Бот не активирован. Автопродажа не может быть включена.\nКлюч: {HASH_MAC}")
                            )
                        else:
                            future = asyncio.run_coroutine_threadsafe(check_wallet_balance(), payment_processor.loop)
                            balance_ton = future.result(timeout=10)
                ...
                c.telegram.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
                    text=sanitize_telegram_text(f"🔐 Текущая мнемоника:\n{current_mnemonic}\n\nВведите новую (24 слова):"),
                    reply_markup=keyboard,
                    parse_mode="HTML"
                )
                ...
                balance_type = "старый формат (без деления на 10^9)" if config["USE_OLD_BALANCE"] else "новый формат (с делением на 10^9)"
                with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
                    json.dump(config, f, indent=4, ensure_ascii=False)
                c.telegram.bot.send_message(
                    chat_id,
